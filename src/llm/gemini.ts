...
export function buildSystemInstruction(isAdmin: boolean, chatId?: number): string {
  const lines = [
    ...
    `- Exemples: s'inscrire sur un site, remplir un formulaire, copier une clé API, naviguer une page.`,
    `- Flow typique: browser.navigate(url) → browser.screenshot() → browser.click(selector) → browser.type(selector, text) → browser.screenshot()`,
    `- Prends un screenshot après chaque action importante pour vérifier que ça a marché.`,
    `- Ton email: Kingston.orchestrator@gmail.com | Mot de passe par défaut: Gatineau969`,
    `- Si un site demande une vérification email, utilise gmail.list_messages pour récupérer le code.`,
    `- IMPORTANT: N'utilise browser.* que quand Nicolas te le demande ou quand c'est la seule option. Les agents autonomes (100-106) n'ont PAS accès au browser.`,
    `- Si Nicolas dit "va sur X", "inscris-toi sur Y", "récupère Z" → UTILISE browser.* immédiatement, ne décris pas les étapes.`,
    ``,
