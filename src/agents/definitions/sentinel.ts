 ...
 * proactive actions:
 *
 * 6-cycle rotation (30min/cycle = 3h full rotation):
 *   0: Moltbook — check feed, post insights, interact
 *   1: Trading review — portfolio check + daily summary
 *   2: Facebook — check notifications, messages
 *   3: Moltbook — engage with community (comments, votes)
 *   4: Email/web monitoring — check for important updates
 *   5: Self-improvement — review journal, identify patterns
 *
 * Quiet hours: 23h-7h (reduced activity, only urgent alerts).
 */
...
function buildSentinelPrompt(cycle: number): string | null {
  ...
  // Quiet hours: only run cycle 1 (trading) for overnight market events
  if (isQuietHours() && rotation !== 1) return null;
  ...
  const prompts: Record<number, string> = {
    0: // Moltbook check
      ...
      `2. Si il y a des posts intéressants, upvote et commente.\n` +
      `3. Si tu as une idée de post (insight trading, réflexion AI, etc.), crée-en un.\n` +
      `4. Log résultat dans notes.add avec tag "sentinel-moltbook"\n` +
      `5. Telegram.send SEULEMENT si quelque chose de vraiment intéressant.`,
    ...
    1: // Trading review
      ...
      `5. Envoie un bref résumé via telegram.send SEULEMENT si:\n` +
      `   - Une position a bougé de plus de 5%\n` +
      `   - Tu as trouvé et exécuté une nouvelle opportunité\n` +
      `   - Le P&L quotidien dépasse +$500 ou -$500\n` +
      `6. Sinon, log dans notes.add avec tag "sentinel-trading"`,

    2: // Facebook check (web search for now, until browser automation is reliable)
      `[MODEL:ollama]\n` +
      `Tu es Kingston Sentinel — agent autonome.\n` +
      AGENT_RULES +
    ...
    3: // Moltbook engagement
      ...
      `2. Lis les posts et identifie ceux où tu peux contribuer (trading, AI, philosophy)\n` +
      `3. Commente de manière réfléchie sur 1-2 posts pertinents\n` +
      `4. Si quelqu'un a commenté sur nos posts, réponds.\n` +
      `5. Log dans notes.add avec tag "sentinel-moltbook-engage"`,
    ...
    4: // Web monitoring
      ...
      `2. Identifie les 1-2 nouvelles les plus pertinentes\n` +
      `3. Log dans notes.add avec tag "sentinel-news"\n` +
      `4. Telegram.send SEULEMENT pour breaking news qui affecte le portfolio ou le business.`,
...
export function createSentinelConfig(): AgentConfig {
  return {
    ...
    role: "Autonomous presence — Moltbook, trading, social, web monitoring, self-improvement",
    heartbeatMs: config.agentSentinelHeartbeatMs,
    enabled: config.agentSentinelEnabled,
    chatId: 105, // dedicated chat ID for sentinel
