/**
 * Custom skill: code.request
 * Queue a code modification request for Claude Code CLI to execute.
 */
...
registerSkill({
  name: "code.request",
  description:
    "Queue a code modification request for Claude Code CLI to execute on next startup.",
  adminOnly: true,
  argsSchema: {
    type: "object",
    properties: {
  ...
  async execute(args): Promise<string> {
    ...
    return (
      ...
      `Priority: ${priority}\n` +
      `Task: ${task}\n` +
      `\nClaude Code CLI will process it on next startup.`
    );
  },
});
